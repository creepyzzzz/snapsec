...
const MetricCard = ({ label, value, sublabel }: { label: string, value: number, sublabel?: string }) => (
    <div className="flex flex-col">
        ...
        <div className="text-xl font-medium text-zinc-900 dark:text-zinc-100 my-0.5">
...
const AssetRow = ({ icon: Icon, name, type, seen, status }: { icon: any, name: string, type: string, seen: string, status: "Active" | "Inactive" | "New" }) => (
    <div className="flex items-center justify-between py-2 border-b border-zinc-100 dark:border-zinc-800/40 last:border-0 group cursor-default">
        <div className="flex items-center gap-3">
            <div className="w-6 h-6 rounded bg-zinc-50 dark:bg-zinc-800/50 flex items-center justify-center border border-zinc-200 dark:border-zinc-700/30 group-hover:border-zinc-300 dark:group-hover:border-zinc-600/50 transition-colors">
                <Icon className="w-3 h-3 text-zinc-400" />
            </div>
            <div className="flex flex-col">
                <span className="text-[11px] font-medium text-zinc-700 dark:text-zinc-300 group-hover:text-zinc-900 dark:group-hover:text-white transition-colors">{name}</span>
                <span className="text-[9px] text-zinc-400 dark:text-zinc-600">{type}</span>
            </div>
        </div>
        <div className="flex items-center gap-4">
            <span className="text-[9px] text-zinc-400 dark:text-zinc-600 font-mono hidden sm:block">{seen}</span>
            <div className={cn("px-1.5 py-0.5 rounded text-[8px] font-medium border",
                status === 'Active' ? 'bg-zinc-100 dark:bg-zinc-800/50 text-zinc-600 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700/50' :
                    status === 'New' ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 border-zinc-900 dark:border-zinc-200 shadow-sm' :
                ...
                <div className="flex-1 p-3 sm:p-4 overflow-y-auto overflow-x-hidden flex flex-col gap-3 text-xs custom-scrollbar">
                    ...
                    <div className="flex items-center justify-between shrink-0">
                        <div className="flex items-center gap-2">
                            <h2 className="text-zinc-900 dark:text-zinc-100 font-medium text-sm tracking-tight">Inventory Overview</h2>
                            <div className="h-4 w-[1px] bg-zinc-200 dark:bg-zinc-800 hidden sm:block" />
                            <p className="text-zinc-400 dark:text-zinc-500 text-[10px] hidden sm:block">Real-time Tracking</p>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-2.5 min-h-0">
                        ...
                        <div className="md:col-span-1 bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/40 rounded-lg p-3 flex flex-col gap-4 backdrop-blur-sm shadow-sm dark:shadow-none transition-all duration-300">
                            <div>
                                ...
                                <div className="space-y-2.5">
                                    <DistributionBar label="Compute" count={4500} total={14205} color="bg-zinc-900 dark:bg-zinc-100" />
                                    <DistributionBar label="Storage" count={3200} total={14205} color="bg-zinc-500" />
                                    <DistributionBar label="Network" count={2800} total={14205} color="bg-zinc-400" />
                                    <DistributionBar label="Database" count={3705} total={14205} color="bg-zinc-200 dark:bg-zinc-700" />
                                </div>
                            </div>
                            ...
                            <div className="mt-auto">
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="text-2xl font-light text-zinc-900 dark:text-white">98.2%</div>
                                    <div className="text-[9px] text-zinc-400 dark:text-zinc-500">Monitored</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="md:col-span-2 bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/40 rounded-lg p-0 overflow-hidden flex flex-col backdrop-blur-sm shadow-sm dark:shadow-none transition-all duration-300">
                            <div className="flex items-center justify-between p-3 border-b border-zinc-100 dark:border-zinc-800/40 bg-zinc-50 dark:bg-zinc-900/20">
                                <h3 className="text-[11px] font-medium text-zinc-700 dark:text-zinc-300">Recently Discovered</h3>
                                <button className="flex items-center gap-1 px-2 py-1 rounded bg-zinc-100 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700/50 text-[9px] text-zinc-500 dark:text-zinc-400 hover:text-zinc-950 dark:hover:text-zinc-200 transition-colors">
